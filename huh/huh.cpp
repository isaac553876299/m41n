/* michael reid 2004 */
/* https://cflmath.com/Rubik/description.html */
...
#define DIST(c, e) (((e) & 0x1) ? (((int)distance[c][(e) >> 1]) >> 4) : (((int)distance[c][(e) >> 1]) & 0xF))
...
/*
The number of nodes overflows on long searches. This can be
fixed by changing type to long long int.
*/
static unsigned int n_nodes;
static unsigned int n_tests;
static int sol_found;

void exit_w_error_message(const char* msg)
{
	printf("\n%s\n", msg);
	exit(EXIT_FAILURE);
}
...
void perm_n_compose(int nn, int perm0_in[], int perm1_in[], int perm_out[])
{
	int ii;
	for (ii = 0; ii < nn; ++ii)
	{
...
void perm_n_conjugate(int nn, int arr_in[], int conjugator[], int array_out[])
{
	int ii;
	for (ii = 0; ii < nn; ++ii)
	{
...
void print_cube2(cube* p_cube)
{
	for (unsigned int i = 0; i < 12; ++i)
	{
		printf("%s ", edge_cubie_str[p_cube->edges[i]]);
	}
	for (unsigned int i = 0; i < 8; ++i)
	{
		printf("%s ", edge_cubie_str[p_cube->corners[i]]);
	}
	printf("\n");
}
...
void cube_conjugate(cube* p_cube_in, cube* p_conjugator, cube* p_cube_out)
{
	perm_n_conjugate(24, p_cube_in->edges, p_conjugator->edges, p_cube_out->edges);
	perm_n_conjugate(24, p_cube_in->corners, p_conjugator->corners, p_cube_out->corners);
}
...
void init_metric(metric_data* p_metric_data, int use_metric)
{
	...
	else //if (p_metric_data->metric == FACE_TURN_METRIC)
	{
...
void init_options(metric_data* p_metric_data, options* p_user_options)
{
	...
	{
		printf("search limit: %d%c\n", p_user_options->search_limit, p_current_metric->metric_char);
	}
...
void init_sym_x_invsym_to_sym()
{
	...
	mem_ptr = (unsigned char(*)[N_SYM])malloc(sizeof(unsigned char[N_SYM][N_SYM]));
	if (mem_ptr == NULL)
	{
	...
	for (ii = 0; ii < N_SYM; ++ii)
	{
		for (jj = 0; jj < N_SYM; ++jj)
		{
			sym_x_invsym_to_sym[ii][jj] = (unsigned char)group_table[ii][group_inverse[jj]];
		}
	}
}
...
void init_invsym_on_twist()
{
	...
	mem_ptr = (unsigned char(*)[N_SYM][N_TWIST])malloc(sizeof(unsigned char[3][N_SYM][N_TWIST]));
	if (mem_ptr == NULL)
	{
	...
	for (sym = 0; sym < N_SYM; ++sym)
	{
		for (twist = 0; twist < N_TWIST; ++twist)
		{
			invsym_on_twist_ud[sym][twist] = (unsigned char)sym_on_twist[(int)sym_x_invsym_to_sym[0][sym]][twist];
		}
	}
...
void generate_subgroup(int group_table[N_CUBESYM][N_CUBESYM], int gen_set[N_CUBESYM])
...
void calc_subgroups_recursive(int group_table[N_CUBESYM][N_CUBESYM],
	int contain_list[N_CUBESYM], int avoid_list[N_CUBESYM], Subgroup_list* p_output)
{
	...
	if (ii < N_CUBESYM)
	{
		...
		calc_subgroups_recursive(group_table, local_contain_list, avoid_list, p_output);
		avoid_list[ii] = 1;
...
{
	if (syllable == SYLLABLE_F || syllable == SYLLABLE_F2 || syllable == SYLLABLE_F3 ||
		syllable == SYLLABLE_R || syllable == SYLLABLE_R2 || syllable == SYLLABLE_R3 ||
		syllable == SYLLABLE_U || syllable == SYLLABLE_U2 || syllable == SYLLABLE_U3 ||
		syllable == SYLLABLE_B || syllable == SYLLABLE_B2 || syllable == SYLLABLE_B3 ||
		syllable == SYLLABLE_L || syllable == SYLLABLE_L2 || syllable == SYLLABLE_L3 ||
		syllable == SYLLABLE_D || syllable == SYLLABLE_D2 || syllable == SYLLABLE_D3)
	{
		return 1;
	}
...
// at current config return syllable-2
...
{
	if ((twists[0] == TWIST_B || twists[0] == TWIST_B2 || twists[0] == TWIST_B3) &&
		(twists[1] == TWIST_F || twists[1] == TWIST_F2 || twists[1] == TWIST_F3))
		return 1;

	if ((twists[0] == TWIST_L || twists[0] == TWIST_L2 || twists[0] == TWIST_L3) &&
		(twists[1] == TWIST_R || twists[1] == TWIST_R2 || twists[1] == TWIST_R3))
		return 1;

	if ((twists[0] == TWIST_D || twists[0] == TWIST_D2 || twists[0] == TWIST_D3) &&
		(twists[1] == TWIST_U || twists[1] == TWIST_U2 || twists[1] == TWIST_U3))
		return 1;
...
int which_subgroup(int subgroup[N_CUBESYM], int subgroup_list[N_SYMSUBGRP][N_CUBESYM])
{
	int subgrp, sym;
...
void calc_syllable_on_sym(int subgroup_list[N_SYMSUBGRP][N_CUBESYM],
	int sym_on_twist[N_CUBESYM][N_TWIST], int syllable_on_sym[N_SYLLABLE][N_SYMSUBGRP])
{
	...
	for (syllable = 0; syllable < N_SYLLABLE; ++syllable)
	{
		...
		else
		{
			...
			for (sym = 0; sym < N_CUBESYM; ++sym)
			{
				...
				if (temp_arr[0] == twist_arr[0] && temp_arr[1] == twist_arr[1])
				{
					subgroup[sym] = 1;
				}
				else
		...
		for (subgrp = 0; subgrp < N_SYMSUBGRP; ++subgrp)
		{
			...
			syllable_on_sym[syllable][subgrp] = which_subgroup(temp_subgroup, subgroup_list);
...
// at current config return twist+2 at else
int twist_on_syllable(int twist, int syllable)
{
	...
	if (twist == TWIST_F)
	{
		if (syllable == SYLLABLE_F || syllable == SYLLABLE_F2 || syllable == SYLLABLE_F3 ||
			syllable == SYLLABLE_B || syllable == SYLLABLE_B2 || syllable == SYLLABLE_B3 ||
			syllable == SYLLABLE_FB || syllable == SYLLABLE_FB2 || syllable == SYLLABLE_FB3 || syllable == SYLLABLE_F2B2)
		{
			return SYLLABLE_INVALID;
		}
		else
	...
	{
		if (syllable == SYLLABLE_R || syllable == SYLLABLE_R2 || syllable == SYLLABLE_R3 ||
			syllable == SYLLABLE_L || syllable == SYLLABLE_L2 || syllable == SYLLABLE_L3 ||
			syllable == SYLLABLE_RL || syllable == SYLLABLE_RL2 || syllable == SYLLABLE_RL3 || syllable == SYLLABLE_R2L2)
		{
			return SYLLABLE_INVALID;
		}
		else
	...
	{
		if (syllable == SYLLABLE_U || syllable == SYLLABLE_U2 || syllable == SYLLABLE_U3 ||
			syllable == SYLLABLE_D || syllable == SYLLABLE_D2 || syllable == SYLLABLE_D3 ||
			syllable == SYLLABLE_UD || syllable == SYLLABLE_UD2 || syllable == SYLLABLE_UD3 || syllable == SYLLABLE_U2D2)
		{
			return SYLLABLE_INVALID;
		}
		else
	...
	{
		if (syllable == SYLLABLE_B || syllable == SYLLABLE_B2 || syllable == SYLLABLE_B3 ||
			syllable == SYLLABLE_FB || syllable == SYLLABLE_FB2 || syllable == SYLLABLE_FB3 || syllable == SYLLABLE_F2B2)
		{
			return SYLLABLE_INVALID;
		}
	...
	{
		if (syllable == SYLLABLE_L || syllable == SYLLABLE_L2 || syllable == SYLLABLE_L3 ||
			syllable == SYLLABLE_RL || syllable == SYLLABLE_RL2 || syllable == SYLLABLE_RL3 || syllable == SYLLABLE_R2L2)
		{
			return SYLLABLE_INVALID;
		}
	...
	{
		if (syllable == SYLLABLE_D || syllable == SYLLABLE_D2 || syllable == SYLLABLE_D3 ||
			syllable == SYLLABLE_UD || syllable == SYLLABLE_UD2 || syllable == SYLLABLE_UD3 || syllable == SYLLABLE_U2D2)
		{
			return SYLLABLE_INVALID;
		}
...
int not_minimal_one_twist(int subgroup[N_CUBESYM], int sym_on_twist[N_CUBESYM][N_TWIST], int twist)
{
	int sym;
...
int not_minimal_two_twists(int subgroup[N_CUBESYM], int sym_on_twist[N_CUBESYM][N_TWIST], int twist0, int twist1)
{
	int twist_arr[2], sym;
	...
	for (sym = 0; sym < N_CUBESYM; ++sym)
	{
		if (subgroup[sym])
		{
			...
			if (twist_arr[0] < twist0 || (twist_arr[0] == twist0 && twist_arr[1] < twist1))
			{
				return 1;
			}
		}
...
void calc_twist_on_sylsubgrp(int tw_on_sylsubgrp[N_TWIST][N_SYLLABLE * N_SYMSUBGRP])
{
	...
	for (syllable = 0; syllable < N_SYLLABLE; ++syllable)
	{
		for (subgrp = 0; subgrp < N_SYMSUBGRP; ++subgrp)
		{
			...
			for (twist = 0; twist < N_TWIST; ++twist)
			{
				...
				{
					if (not_minimal_one_twist(subgroup_list[subgrp_after], sym_on_twist, syllable_to_twist(new_syllable)))
					{
				...
				{
					if (not_minimal_two_twists(subgroup_list[subgrp_before], sym_on_twist, syllable_to_twist(syllable), twist))
					{
						new_syllable = SYLLABLE_INVALID;
					}
				}
				...
				tw_on_sylsubgrp[twist][syllable * N_SYMSUBGRP + subgrp] = new_syllable * N_SYMSUBGRP + new_subgrp;
			}
		}
	}
}

void init_twist_on_follow()
{
	...
	while (found)
	{
		...
		for (syllable = 0; syllable < N_SYLLABLE; ++syllable)
		{
			for (subgrp = 0; subgrp < N_SYMSUBGRP; ++subgrp)
			{
				for (twist = 0; twist < N_TWIST; ++twist)
				{
					if (occurs[tw_on_sylsubgrp[twist][syllable * N_SYMSUBGRP + subgrp]] == 0)
					{
						occurs[tw_on_sylsubgrp[twist][syllable * N_SYMSUBGRP + subgrp]] = 1;
						found = 1;
					}
				}
			}
	...
	for (subgrp = 0; subgrp < N_SYMSUBGRP; ++subgrp)
	{
		if (sylsubgrp_to_follow[SYLLABLE_NONE * N_SYMSUBGRP + subgrp] != 1 + subgrp)
		{
			exit_w_error_message("init_twist_on_follow : indexing error");
		}
	}

	mem_ptr = (unsigned short(*)[N_FOLLOW])malloc(sizeof(unsigned short[N_TWIST][N_FOLLOW]));
	if (mem_ptr == NULL)
	{
	...
	for (twist = 0; twist < N_TWIST; ++twist)
	{
		...
		{
			twist_on_follow[twist][follow] = (unsigned short)sylsubgrp_to_follow[tw_on_sylsubgrp[twist][follow_to_sylsubgrp[follow]]];
		}
	}
}
...
void corner_unpack(int corner, int array_out[8])
{
	...
	array_out[7] = (2 * (array_out[0] + array_out[1] + array_out[2] + array_out[3] + array_out[4] + array_out[5] + array_out[6])) % 3;
}
...
void corner_adjust(int array[8], int ind0, int ind1, int ind2, int ind3)
{
...
void init_twist_on_corner() 
{
	int twist, corner;

	/* allocate and initialize global array twist_on_corner */

	twist_on_corner[0] = (unsigned short*)malloc(sizeof(unsigned short[N_TWIST][N_CORNER]));
	...
	{
		twist_on_corner[TWIST_F][corner] = (unsigned short)twist_f_on_corner(corner);
		twist_on_corner[TWIST_R][corner] = (unsigned short)twist_r_on_corner(corner);
		twist_on_corner[TWIST_U][corner] = (unsigned short)twist_u_on_corner(corner);
		twist_on_corner[TWIST_B][corner] = (unsigned short)twist_b_on_corner(corner);
		twist_on_corner[TWIST_L][corner] = (unsigned short)twist_l_on_corner(corner);
		twist_on_corner[TWIST_D][corner] = (unsigned short)twist_d_on_corner(corner);
	}
	for (corner = 0; corner < N_CORNER; ++corner)
	{
		twist_on_corner[TWIST_F2][corner] = twist_on_corner[TWIST_F][(int)twist_on_corner[TWIST_F][corner]];
		twist_on_corner[TWIST_R2][corner] = twist_on_corner[TWIST_R][(int)twist_on_corner[TWIST_R][corner]];
		twist_on_corner[TWIST_U2][corner] = twist_on_corner[TWIST_U][(int)twist_on_corner[TWIST_U][corner]];
		twist_on_corner[TWIST_B2][corner] = twist_on_corner[TWIST_B][(int)twist_on_corner[TWIST_B][corner]];
		twist_on_corner[TWIST_L2][corner] = twist_on_corner[TWIST_L][(int)twist_on_corner[TWIST_L][corner]];
		twist_on_corner[TWIST_D2][corner] = twist_on_corner[TWIST_D][(int)twist_on_corner[TWIST_D][corner]];
	}
	for (corner = 0; corner < N_CORNER; ++corner)
	{
		twist_on_corner[TWIST_F3][corner] = twist_on_corner[TWIST_F2][(int)twist_on_corner[TWIST_F][corner]];
		twist_on_corner[TWIST_R3][corner] = twist_on_corner[TWIST_R2][(int)twist_on_corner[TWIST_R][corner]];
		twist_on_corner[TWIST_U3][corner] = twist_on_corner[TWIST_U2][(int)twist_on_corner[TWIST_U][corner]];
		twist_on_corner[TWIST_B3][corner] = twist_on_corner[TWIST_B2][(int)twist_on_corner[TWIST_B][corner]];
		twist_on_corner[TWIST_L3][corner] = twist_on_corner[TWIST_L2][(int)twist_on_corner[TWIST_L][corner]];
		twist_on_corner[TWIST_D3][corner] = twist_on_corner[TWIST_D2][(int)twist_on_corner[TWIST_D][corner]];
	}
}
...
void init_sym_on_corner()
{
	int corner, sym;

	/* allocate and initialize global array sym_on_corner */

	sym_on_corner[0] = (unsigned short*)malloc(sizeof(unsigned short[N_SYM][N_CORNER]));
	...
	for (sym = 2; sym < 4; ++sym)
	{
		for (corner = 0; corner < N_CORNER; ++corner)
		{
			sym_on_corner[sym][corner] = sym_on_corner[1][(int)sym_on_corner[sym - 1][corner]];
		}
	}
	...
	for (sym = 5; sym < 8; ++sym)
	{
		for (corner = 0; corner < N_CORNER; ++corner)
		{
			sym_on_corner[sym][corner] = sym_on_corner[4][(int)sym_on_corner[sym - 4][corner]];
		}
	}
	...
	for (sym = 9; sym < 16; ++sym)
	{
		for (corner = 0; corner < N_CORNER; ++corner)
		{
			sym_on_corner[sym][corner] = sym_on_corner[8][(int)sym_on_corner[sym - 8][corner]];
		}
	}
}
		...
		for (loc1 = loc0 + 1; loc1 < 10; ++loc1)
		{
			for (loc2 = loc1 + 1; loc2 < 11; ++loc2)
			{
				for (loc3 = loc2 + 1; loc3 < 12; ++loc3)
				{
					...
					conv_tab[count] = (1 << loc0) | (1 << loc1) | (1 << loc2) | (1 << loc3);
...
void eflip_adjust(int array[12], int ind0, int ind1, int ind2, int ind3)
{
...
void init_fulledge_to_edgequot(
	...
	int multiplicities[N_EDGEQUOT])
{
	...
	fulledge_to_edge = (unsigned short*)malloc(sizeof(unsigned short[N_FULLEDGE]));
	...
	for (fulledge = 0; fulledge < N_FULLEDGE; ++fulledge)
	{
		...
		{
			new_full = sym_on_eloc[2 * sym][fulledge / N_EFLIP] * N_EFLIP + sym_on_eflip[sym][fulledge % N_EFLIP];
			...
			new_full = sym_on_eloc[2 * sym][cu_fulledge / N_EFLIP] * N_EFLIP + sym_on_eflip[sym][cu_fulledge % N_EFLIP];
			if (min_full > new_full)
			{
				min_full = new_full;
				min_sym = 2 * sym + 1;
		...
		if (min_sym == 0)
		{
			if (count >= N_EDGEQUOT)
			{
				exit_w_error_message("init_fulledge_to_edgequot : too many  edgequot's");
			}
	...
	if (count != N_EDGEQUOT)
	{
		exit_w_error_message("init_fulledge_to_edgequot : wrong number of  edgequot's");
	}
}
	...
	int edge_to_fulledge[N_EDGEQUOT])
{
	...
	twist_on_edge[0] = (unsigned short*)malloc(sizeof(unsigned short[N_TWIST][N_EDGEQUOT]));
	twist_x_edge_to_sym[0] = (unsigned char*)malloc(sizeof(unsigned char[N_TWIST][N_EDGEQUOT]));
	...
	for (twist = 0; twist < N_TWIST; ++twist)
	{
		for (edge = 0; edge < N_EDGEQUOT; ++edge)
		{
			...
			new_edge = twist_on_eloc[twist][fulledge / N_EFLIP] * N_EFLIP + twist_on_eflip[twist][fulledge % N_EFLIP];
			twist_on_edge[twist][edge] = fulledge_to_edge[new_edge];
			twist_x_edge_to_sym[twist][edge] = sym_x_invsym_to_sym[0][(int)fulledge_to_sym[new_edge]];
		}
	}
}

void init_edge_quotient(int stabilizers[N_EDGEQUOT], int multiplicities[N_EDGEQUOT])
{
	...
	init_fulledge_to_edgequot(sym_on_eloc, sym_on_eflip, edge_to_fulledge, stabilizers, multiplicities);
...
void init_twist_on_cornerperm()
{
	int cp, twist;

	/* allocate and initialize global array twist_on_cornerperm */

	twist_on_cornerperm[0] = (unsigned short*)malloc(sizeof(unsigned short[N_TWIST][N_CORNERPERM]));
	...
	for (cp = 0; cp < N_CORNERPERM; ++cp)
	{
		twist_on_cornerperm[TWIST_F2][cp] = twist_on_cornerperm[TWIST_F][(int)twist_on_cornerperm[TWIST_F][cp]];
		twist_on_cornerperm[TWIST_R2][cp] = twist_on_cornerperm[TWIST_R][(int)twist_on_cornerperm[TWIST_R][cp]];
		twist_on_cornerperm[TWIST_U2][cp] = twist_on_cornerperm[TWIST_U][(int)twist_on_cornerperm[TWIST_U][cp]];
		twist_on_cornerperm[TWIST_B2][cp] = twist_on_cornerperm[TWIST_B][(int)twist_on_cornerperm[TWIST_B][cp]];
		twist_on_cornerperm[TWIST_L2][cp] = twist_on_cornerperm[TWIST_L][(int)twist_on_cornerperm[TWIST_L][cp]];
		twist_on_cornerperm[TWIST_D2][cp] = twist_on_cornerperm[TWIST_D][(int)twist_on_cornerperm[TWIST_D][cp]];
	}
	for (cp = 0; cp < N_CORNERPERM; ++cp)
	{
		twist_on_cornerperm[TWIST_F3][cp] = twist_on_cornerperm[TWIST_F2][(int)twist_on_cornerperm[TWIST_F][cp]];
		twist_on_cornerperm[TWIST_R3][cp] = twist_on_cornerperm[TWIST_R2][(int)twist_on_cornerperm[TWIST_R][cp]];
		twist_on_cornerperm[TWIST_U3][cp] = twist_on_cornerperm[TWIST_U2][(int)twist_on_cornerperm[TWIST_U][cp]];
		twist_on_cornerperm[TWIST_B3][cp] = twist_on_cornerperm[TWIST_B2][(int)twist_on_cornerperm[TWIST_B][cp]];
		twist_on_cornerperm[TWIST_L3][cp] = twist_on_cornerperm[TWIST_L2][(int)twist_on_cornerperm[TWIST_L][cp]];
		twist_on_cornerperm[TWIST_D3][cp] = twist_on_cornerperm[TWIST_D2][(int)twist_on_cornerperm[TWIST_D][cp]];
	}
}
...
void init_twist_on_sliceedge()
{
	int twist, sl;

	/* allocate and initialize global array twist_on_sliceedge */

	twist_on_sliceedge[0] = (unsigned short*)malloc(sizeof(unsigned short[N_TWIST][N_SLICEEDGE]));
	...
	for (sl = 0; sl < N_SLICEEDGE; ++sl)
	{
		twist_on_sliceedge[TWIST_F2][sl] = twist_on_sliceedge[TWIST_F][(int)twist_on_sliceedge[TWIST_F][sl]];
		twist_on_sliceedge[TWIST_R2][sl] = twist_on_sliceedge[TWIST_R][(int)twist_on_sliceedge[TWIST_R][sl]];
		twist_on_sliceedge[TWIST_U2][sl] = twist_on_sliceedge[TWIST_U][(int)twist_on_sliceedge[TWIST_U][sl]];
		twist_on_sliceedge[TWIST_B2][sl] = twist_on_sliceedge[TWIST_B][(int)twist_on_sliceedge[TWIST_B][sl]];
		twist_on_sliceedge[TWIST_L2][sl] = twist_on_sliceedge[TWIST_L][(int)twist_on_sliceedge[TWIST_L][sl]];
		twist_on_sliceedge[TWIST_D2][sl] = twist_on_sliceedge[TWIST_D][(int)twist_on_sliceedge[TWIST_D][sl]];
	}
	for (sl = 0; sl < N_SLICEEDGE; ++sl)
	{
		twist_on_sliceedge[TWIST_F3][sl] = twist_on_sliceedge[TWIST_F2][(int)twist_on_sliceedge[TWIST_F][sl]];
		twist_on_sliceedge[TWIST_R3][sl] = twist_on_sliceedge[TWIST_R2][(int)twist_on_sliceedge[TWIST_R][sl]];
		twist_on_sliceedge[TWIST_U3][sl] = twist_on_sliceedge[TWIST_U2][(int)twist_on_sliceedge[TWIST_U][sl]];
		twist_on_sliceedge[TWIST_B3][sl] = twist_on_sliceedge[TWIST_B2][(int)twist_on_sliceedge[TWIST_B][sl]];
		twist_on_sliceedge[TWIST_L3][sl] = twist_on_sliceedge[TWIST_L2][(int)twist_on_sliceedge[TWIST_L][sl]];
		twist_on_sliceedge[TWIST_D3][sl] = twist_on_sliceedge[TWIST_D2][(int)twist_on_sliceedge[TWIST_D][sl]];
	}
}
...
void init_distance_table(int edge_stabilizers[N_EDGEQUOT], int edge_multiplicities[N_EDGEQUOT])
{
	...
	{
		printf("%7d%c %13d     (%8d)\n",
			sdata_struc.depth, p_current_metric->metric_char, sdata_struc.found, sdata_struc.found_quot);
		...
		else  /* search backward */
		{
			...
			for (corner = 0; corner < N_CORNER; ++corner)
			{
				for (edge = 0; edge < N_EDGEQUOT; ++edge)
				{
					if ((DIST(corner, edge) == 15) && neighbors_are_previous(corner, edge, &sdata_struc))
					{
...
/* output: *p_coset_coord */
{
	...
	p_coset_coord->corner_state = (int)sym_on_corner[(int)sym_x_invsym_to_sym[0][sym]][corner];
}
...
void set_cube_symmetry(full_cube* p_cube, cube sym_cubes[N_CUBESYM], int subgroup_list[N_SYMSUBGRP][N_CUBESYM])
{
	cube temp_cube;
...
void search_tree(full_cube* p_cube, search_node* node_arr)
{
	...
	while (p_node >= node_arr)
	{
		...
		{
			if (test_for_solution(p_cube, node_arr) && p_current_options->one_solution_only)
			{
				return;
			}
			--p_node;
		...
		else
		{
			for (twist = p_node[1].twist + 1; twist < N_TWIST; ++twist)
			{
				...
				p_node[1].remain_depth = p_node->remain_depth - p_current_metric->twist_length[twist];
				...
				virtual_twist = (int)invsym_on_twist_ud[p_node->ud.sym_state][twist];
				new_sym_factor = (int)twist_x_edge_to_sym[virtual_twist][p_node->ud.edge_state];
				p_node[1].ud.edge_state = (int)twist_on_edge[virtual_twist][p_node->ud.edge_state];
				p_node[1].ud.sym_state = (int)sym_x_invsym_to_sym[p_node->ud.sym_state][new_sym_factor];
				p_node[1].ud.corner_state = (int)sym_on_corner[new_sym_factor][(int)twist_on_corner[virtual_twist][p_node->ud.corner_state]];

				if (p_node[1].remain_depth < DIST(p_node[1].ud.corner_state, p_node[1].ud.edge_state))
				{
					continue;
				}


				virtual_twist = (int)invsym_on_twist_rl[p_node->rl.sym_state][twist];
				new_sym_factor = (int)twist_x_edge_to_sym[virtual_twist][p_node->rl.edge_state];
				p_node[1].rl.edge_state = (int)twist_on_edge[virtual_twist][p_node->rl.edge_state];
				p_node[1].rl.sym_state = (int)sym_x_invsym_to_sym[p_node->rl.sym_state][new_sym_factor];
				p_node[1].rl.corner_state = (int)sym_on_corner[new_sym_factor][(int)twist_on_corner[virtual_twist][p_node->rl.corner_state]];

				if (p_node[1].remain_depth < DIST(p_node[1].rl.corner_state, p_node[1].rl.edge_state))
				{
					continue;
				}


				virtual_twist = (int)invsym_on_twist_fb[p_node->fb.sym_state][twist];
				new_sym_factor = (int)twist_x_edge_to_sym[virtual_twist][p_node->fb.edge_state];
				p_node[1].fb.edge_state = (int)twist_on_edge[virtual_twist][p_node->fb.edge_state];
				p_node[1].fb.sym_state = (int)sym_x_invsym_to_sym[p_node->fb.sym_state][new_sym_factor];
				p_node[1].fb.corner_state = (int)sym_on_corner[new_sym_factor][(int)twist_on_corner[virtual_twist][p_node->fb.corner_state]];

				if (p_node[1].remain_depth < DIST(p_node[1].fb.corner_state, p_node[1].fb.edge_state))
				{
					continue;
				}
...
void solve_cube(cube* p_cube)
{
	...
	sol_found = 0;
	if (p_current_metric->metric == QUARTER_TURN_METRIC && full_cube_struct.parity == 0)
	{
		start_depth = 2;
	}
	else
...
int main()
{
	...
	while (1)
	{
		...
		cube_init(&cube_struct);
		stat = user_twists_cube(&cube_struct);
		...
		if (stat == 0)
		{
			...
			print_cube(&cube_struct);
			print_inverse_cube(&cube_struct);
		}
	}
...
int twist__on_corner(const char& ch, int corner)
{
	int temp_arr[8];
	...
	switch (ch)
	{
	case 'f': four_cycle(temp_arr, CORNER_DFL, CORNER_DRF, CORNER_UFR, CORNER_ULF);
		corner_adjust(temp_arr, CORNER_DFL, CORNER_DRF, CORNER_UFR, CORNER_ULF); break;
	case 'r': four_cycle(temp_arr, CORNER_DRF, CORNER_DBR, CORNER_URB, CORNER_UFR);
		corner_adjust(temp_arr, CORNER_DRF, CORNER_DBR, CORNER_URB, CORNER_UFR); break;
	case 'u': four_cycle(temp_arr, CORNER_UFR, CORNER_URB, CORNER_UBL, CORNER_ULF); break;
	case 'b': four_cycle(temp_arr, CORNER_DBR, CORNER_DLB, CORNER_UBL, CORNER_URB);
		corner_adjust(temp_arr, CORNER_DBR, CORNER_DLB, CORNER_UBL, CORNER_URB); break;
	case 'l': four_cycle(temp_arr, CORNER_DLB, CORNER_DFL, CORNER_ULF, CORNER_UBL);
		corner_adjust(temp_arr, CORNER_DLB, CORNER_DFL, CORNER_ULF, CORNER_UBL); break;
	case 'd': four_cycle(temp_arr, CORNER_DFL, CORNER_DLB, CORNER_DBR, CORNER_DRF); break;
	}
...
int twist__on_eloc(const char& ch, int eloc)
{
	int temp_arr[12];
	...
	switch (ch)
	{
	case 'f': four_cycle(temp_arr, EDGE_FL, EDGE_DF, EDGE_FR, EDGE_UF); break;
	case 'r': four_cycle(temp_arr, EDGE_FR, EDGE_DR, EDGE_BR, EDGE_UR); break;
	case 'u': four_cycle(temp_arr, EDGE_UF, EDGE_UR, EDGE_UB, EDGE_UL); break;
	case 'b': four_cycle(temp_arr, EDGE_BR, EDGE_DB, EDGE_BL, EDGE_UB); break;
	case 'l': four_cycle(temp_arr, EDGE_BL, EDGE_DL, EDGE_FL, EDGE_UL); break;
	case 'd': four_cycle(temp_arr, EDGE_DF, EDGE_DL, EDGE_DB, EDGE_DR); break;
	}
...
int twist__on_eflip(const char& ch, int eflip)
{
	int temp_arr[12];
	...
	switch (ch)
	{
	case 'f': four_cycle(temp_arr, EDGE_FL, EDGE_DF, EDGE_FR, EDGE_UF);
		eflip_adjust(temp_arr, EDGE_FL, EDGE_DF, EDGE_FR, EDGE_UF); break;
	case 'r': four_cycle(temp_arr, EDGE_FR, EDGE_DR, EDGE_BR, EDGE_UR); break;
	case 'u': four_cycle(temp_arr, EDGE_UR, EDGE_UB, EDGE_UL, EDGE_UF); break;
	case 'b': four_cycle(temp_arr, EDGE_BR, EDGE_DB, EDGE_BL, EDGE_UB);
		eflip_adjust(temp_arr, EDGE_BR, EDGE_DB, EDGE_BL, EDGE_UB); break;
	case 'l': four_cycle(temp_arr, EDGE_BL, EDGE_DL, EDGE_FL, EDGE_UL); break;
	case 'd': four_cycle(temp_arr, EDGE_DL, EDGE_DB, EDGE_DR, EDGE_DF); break;
	}
...
int twist__on_cperm(const char& ch, int cperm)
{
	int temp_arr[8];
	...
	switch (ch)
	{
	case 'f': four_cycle(temp_arr, CORNER_DFL, CORNER_DRF, CORNER_UFR, CORNER_ULF); break;
	case 'r': four_cycle(temp_arr, CORNER_DRF, CORNER_DBR, CORNER_URB, CORNER_UFR); break;
	case 'u': four_cycle(temp_arr, CORNER_URB, CORNER_UBL, CORNER_ULF, CORNER_UFR); break;
	case 'b': four_cycle(temp_arr, CORNER_DBR, CORNER_DLB, CORNER_UBL, CORNER_URB); break;
	case 'l': four_cycle(temp_arr, CORNER_DLB, CORNER_DFL, CORNER_ULF, CORNER_UBL); break;
	case 'd': four_cycle(temp_arr, CORNER_DFL, CORNER_DLB, CORNER_DBR, CORNER_DRF); break;
	}
...
int twist__on_sliceedge(const char& ch, int sliceedge)
{
	int temp_arr[12];
