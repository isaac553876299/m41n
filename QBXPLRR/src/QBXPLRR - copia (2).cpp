...
typedef enum Corner { URF, UFL, ULB, UBR, DFR, DLF, DBL, DRB };
Corner& operator++(Corner& corner)
{
	return corner = static_cast<Corner>(static_cast<int>(corner) + 1);
}
Corner& operator++(Corner& corner, int)
{
...
typedef enum Edge { UR, UF, UL, UB, DR, DF, DL, DB, FR, FL, BL, BR };
Edge& operator++(Edge& edge)
{
	return edge = static_cast<Edge>(static_cast<int>(edge) + 1);
}
Edge& operator++(Edge& edge, int)
{
	Edge edge2 = edge;
	++edge;
	return edge2;
...
{
	return move = static_cast<Move>(static_cast<int>(move) + 1);
}
Move& operator++(Move& move, int)
{
	Move move2 = move;
	++move;
	return move2;
...
{
	return axis = static_cast<Axis>(static_cast<int>(axis) + 1);
}
Axis& operator++(Axis& axis, int)
{
	Axis axis2 = axis;
	++axis;
	return axis2;
...
typedef struct faceletCube { Color f[54]; } FaceletCube;
...
typedef struct cubieCube { struct corner_o co[8]; struct edge_o eo[12]; } CubieCube;
