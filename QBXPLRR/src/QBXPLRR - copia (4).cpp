...
CubieCube invCubieCube(CubieCube cc)
{
	...
	for (int edg = UR; edg <= BR; edg++) ccInv.eo[cc.eo[edg].e].e = static_cast<Edge>(edg);
	for (int edg = UR; edg <= BR; edg++) ccInv.eo[edg].o = cc.eo[ccInv.eo[edg].e].o;

	for (int crn = URF; crn <= DRB; crn++) ccInv.co[cc.co[crn].c].c = static_cast<Corner>(crn);
...
unsigned long long getSymmetries(CubieCube cc)
{
	...
	for (symIdx = 0; symIdx < 48; symIdx++)
	{
		...
		for (int i = URF; i <= DRB; i++) if (ccTmp.co[i].c != cc.co[i].c || ccTmp.co[i].o != cc.co[i].o) { hasThisSym = 0; break; }
		if (!hasThisSym) continue;
		for (int j = UR; j <= BR; j++) if (ccTmp.eo[j].e != cc.eo[j].e || ccTmp.eo[j].o != cc.eo[j].o) { hasThisSym = 0; break; }
