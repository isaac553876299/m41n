...
enum Myass { HI, BA, GA };
Myass& operator++(Myass& myass) { return myass = static_cast<Myass>(static_cast<int>(myass) + 1); }
Myass& operator++(Myass& myass, int) { Myass myass2 = myass; ++myass; return myass2; }
...
// Mapping the Facelets to the FaceletColor
...
// The result of the multiplication is the composition of the the permutations a and b.
// Because we also describe reflections of the whole cube by permutations, we get a complication with the corners.
// The orientation of mirrored corners is described by the numbers 3, 4 and 5.
// The composition of the orientations cannot be computed by addition modulo three in the cyclic group C3 any more.
// Instead the rules below give an addition in the dihedral group D3 with 6 elements.
{
	char ori, oriA, oriB;
	ori = 0;
	for (int corner = URF; corner <= DRB; corner++)
	{
		ab->co[corner].c = a->co[b->co[corner].c].c;
		oriA = a->co[b->co[corner].c].o;
		oriB = b->co[corner].o;
...
// Computes the composition of the permutations a and b including orientations.
// With edges we have no complications with mirror images, because the mirror image of an edgecubie looks the same as the edgecubie itself.
...
{
	for (int edge = UR; edge <= BR; edge++)
	{
		ab->eo[edge].e = a->eo[b->eo[edge].e].e;
		ab->eo[edge].o = (b->eo[edge].o + a->eo[b->eo[edge].e].o) % 2;
	};
}

// Do a U,R,F,D,L, or B move on the CubieCube level.
CubieCube cubeAxMove(CubieCube cc, int axis)
{
	CubieCube ccRet;
	cornerMultiply(&cc, &basicCubeMove[axis], &ccRet);
	edgeMultiply(&cc, &basicCubeMove[axis], &ccRet);
	return ccRet;
};
...
{
	CubieCube cc;
	for (int axis = U; axis <= B; axis++)
	{
		cc = moveCube[2 * axis] = basicCubeMove[axis];//ax
		cc = cubeAxMove(cc, axis);//ax^2
		moveCube[2 * axis + 1] = cubeAxMove(cc, axis);//ax^3
	}
}
...
{
	short int ret = 0;
	for (int c = URF; c < DRB; c++) ret = 3 * ret + cc.co[c].o;
	return ret;
}
...
{
	short int ret = 0;
	for (int e = UR; e < BR; e++) ret = 2 * ret + cc.eo[e].o;
	return ret;
}
...
// Initializes the twistMove table which gives the new twist of a cube A with twist a after applying move m.
...
{
	int symIdx = 0, classIdx, rawFlipSlice, rep = NFLIP * NSLICE, rBound, lBound;
	CubieCube ccRep;
	for (int i = 0; i < NSYM_D4h; i++)
	{
...
// Because there are 64430*16 different symFlipSlice coordinates, the value is computed by a function and not realized as a table like twistMove.
int symFlipSliceMove(int a, int m)
{
	int aSym, aClass, bSym, bClass, mConj, b;
	// A = invSymFlipSlice(a), M = moveCube[m] and ASYM = symCube[aSym]
	// A = ASYM^-1*ACLASS*ASYM with some representant ACLASS so A*M = ASYM^-1*ACLASS*ASYM*M = ASYM^-1*ACLASS*(ASYM *M*ASYM^-1)*ASYM
	// MCONJ = ASYM *M*ASYM^-1 then A*M = ASYM^-1*ACLASS*MCONJ*ASYM
	// B = ACLASS*MCONJ then A*M = ASYM^-1*B*ASYM
	// BSYM = symCube[bSym] then B = BSYM^-1*BCLASS*BSYM with some representant BCLASS
	// So A*M = ASYM^-1* BSYM^-1*BCLASS*BSYM*ASYM = (BSYM*ASYM)^-1*BCLASS*(BSYM*ASYM)
	aClass = a >> 4; aSym = a & 15;
	mConj = moveConjugate[m][aSym];
	b = symFlipSliceClassMove[aClass][moveConjugate[m][aSym]];
	bClass = b >> 4; bSym = b & 15;
	return (bClass << 4) + symIdxMultiply[bSym][aSym];

	/*b = symFlipSliceClassMove[(a >> 4)][moveConjugate[m][(a & 15)]];
	return ((b >> 4) << 4) + symIdxMultiply[(b & 15)][(a & 15)];*/
}
...
// Initializes the 48 cube symmetries of the cube on the CubieCube level at symCube[].
void initSymCubes()
{
	int a, b, c, d, idx = 0;
...
// Initializes the index of the inverse symmetry table at invSymIdx[]:
...
void initSymIdxMultiply()
{
	...
	for (int i = 0; i < NSYM_Oh; i++)
	{
		for (int j = 0; j < NSYM_Oh; j++)
		{
			...
			{
				if (symCube[k].eo[UR].e == cc.eo[UR].e && symCube[k].eo[UF].e == cc.eo[UF].e)
				{
...
// Initializes the moveConjugate table. If m is a move and x the index of a symmetry,
// moveConjugate[m][x] gives the move m' which is the conjugation of move m by the the symmetry x:
...
// Initializes the twistConjugate table. If A is a cube with twist t and s is a symmetry,
// twistConjugate[t][s] gives the twist t' of the conjugate cube.
...
void initTwistConjugate()
{
	...
	{
		cc = invTwist(i);
		for (int j = 0; j < NSYM_D4h; j++)
		{
			twistConjugate[i][j] = twist(cornerConjugate(cc, j));
		}
	}
}


// Two cubes A and B are called equivalent, if conjugation of one cube by a symmetry s in D4h (16 symmetries which preserve the UD-axis)
// gives the other cube: symCube[s]*A*symCube[s]^-1 = B
// The rawflipSlice coordinate is computed by 2048*slice + flip.
// The symFlipSliceRep table maps the index x < 64430 of a flipSlice equivalence class to the smallest rawflipSlice coordinate of all cubes in this class.
// In other words rawFlipSliceRep[x] gives the rawFlipSlice coordinate of the representant of this class. 
void initRawFLipSliceRep()
{
	int i, j, k, n, min, idx = 0;
	CubieCube ccK, ccJ, ccI;
	bool flag[NSLICE * NFLIP];
	std::fill(flag, &flag[NSLICE * NFLIP - 1], false);
	//for (j = 0; j < NSLICE * NFLIP; j++) flag[j] = false;
	for (j = 0; j < NSLICE; j++)
	{
		ccJ = invSlice(j);
		for (k = 0; k < NFLIP; k++)
		{
			min = NFLIP * j + k;
			if (flag[min] == true) continue;//no representant
			ccK = invFlip(k);
...
// Computes the symmetries of the cube cc. This is relevant if we want to reduce the maneuver searchspace exploiting the symmetries of the given cube.
unsigned long long getSymmetries(CubieCube cc)
{
	bool hasThisSym;
	unsigned long long symRet = 0;
	CubieCube ccTmp1, ccTmp;
	for (int symIdx = 0; symIdx < 48; symIdx++)
	{
		hasThisSym = true;
		...
		for (int i = URF; i <= DRB; i++) if (ccTmp.co[i].c != cc.co[i].c || ccTmp.co[i].o != cc.co[i].o) { hasThisSym = false; break; }
		if (!hasThisSym) continue;
		for (int j = UR; j <= BR; j++) if (ccTmp.eo[j].e != cc.eo[j].e || ccTmp.eo[j].o != cc.eo[j].o) { hasThisSym = false; break; }
...
// Initializes the ESymmetries and GESymmetries table.
// initESymmetries[m] is a 48 bit vector where bit s is set if moveConjugate[m][s]=m, in other words m is a fixpoint of the symmetry s.
// GESymmetries is similar to ESymmetries. The bit s is set if moveConjugate[m][s]>=m.
// These table are needed if we want to reduce the searchspace by symmetry in the case the cube we solve has symmetry itself.
void initGESymmetries()
{
	...
	for (int i = 0; i < NSYM_Oh; i++)
	{
		for (m = mU1; m <= mB3; m++)
		{
			if (i == 0) { ESymmetries[m] = 0; GESymmetries[m] = 0; }
...
// Computes the movesCloserToTarget table which acts as a pruning table. The target subgroup has the following property:
// the twist coordinate is 0 (all corner twist are ok), the flip cooordinate is 0 (all edge flips are ok),
// the UD-slice edges are in their slice and the edge/corner parity is even.
// Every coset is indexed by its symFlipSlice coordinate (16*classIndex + symmetryIndex), its twist coordinate and its parity.
// movesCloserToTarget[twist][2*classIndex+parity] is a short int where bit m with 0<=m<12 is set when move m carries the cube closer to the target subgroup. 
void initMovesCloserToTarget()
{
	FILE* tableSave;
	char fileName[] = { "pruntable" };
	int flipSlice, twist, parity, i;
...
#pragma warning(suppress : 4996)
	...
	std::cout << "loading pruning table (538 MB) from disk...";
	for (i = 0; i < NTWIST; i++)
	{
		if (!(i % 200)) pp();
...
MAKETABLE:
	std::cout << "generating pruning table(538 MB), this may take half an hour." << std::endl;
			...
			for (twist = 0; twist < NTWIST; twist++)
				for (parity = 0; parity < 2; parity++)
				{
					...
					else
					{
						for (m = mU1; m <= mB3; m++)
						{
							idx1 = ((NTWIST * (symFlipSliceClassMove[flipSlice][m] >> 4) + twistConjugate[twistMove[twist][m]][(symFlipSliceClassMove[flipSlice][m] & 15)]) << 1) + (parity ^ 1);// parity^1: quarter turn changes parity
	...
	std::cout << "saving pruning table (538 MB) to disk..." << std::endl;
#pragma warning(suppress : 4996)
	tableSave = fopen(fileName, "w+b");
	for (i = 0; i < NTWIST; i++) fwrite(movesCloserToTarget[i], NFLIPSLICE * 4, 1, tableSave);
	fclose(tableSave);
	free(visitedA); free(visitedB);
	std::cout << " done.\non the next run the table will be loaded from disk." << std::endl;
}

// movesCloserToTarget applies only if symmetryIndex s=0. For s<>0 the corresponding moves have to be conjugated by symCube(s).
...
// Computes the nextMove table. Given an 12-bit movesAllowed vector z and a move m, nextMove[z][m] returns the next move m' in z with m'>m.
...
void solveOptimal(CubieCube cu)
{
	...
	snP->movesCloserTargetU = moveBitsConjugate[movesCloserToTarget[twistConjU][(snP->flipSliceU << 1) + snP->parity]][snP->symU];
	...
	snP->movesCloserTargetR = moveBitsConjugate[movesCloserToTarget[twistConjR][(snP->flipSliceR << 1) + snP->parity]][symIdxMultiply[snP->symR][16]];
	snP->movesCloserTargetF = moveBitsConjugate[movesCloserToTarget[twistConjF][(snP->flipSliceF << 1) + snP->parity]][symIdxMultiply[snP->symF][32]];
...
MOVE:
	...
	snPNew->distU = snP->distU + ((snP->movesCloserTargetU & 1 << m) ? (-1) : 1);
	snPNew->distR = snP->distR + ((snP->movesCloserTargetR & 1 << m) ? (-1) : 1);
	snPNew->distF = snP->distF + ((snP->movesCloserTargetF & 1 << m) ? (-1) : 1);
	...
	// 12bit vectors hold information which moves carry the cube closer to the target group.
	snPNew->movesCloserTargetU = moveBitsConjugate[movesCloserToTarget[twistConjU][(snPNew->flipSliceU << 1) + snPNew->parity]][snPNew->symU];
	snPNew->movesCloserTargetR = moveBitsConjugate[movesCloserToTarget[twistConjR][(snPNew->flipSliceR << 1) + snPNew->parity]][symIdxMultiply[snPNew->symR][16]];
	snPNew->movesCloserTargetF = moveBitsConjugate[movesCloserToTarget[twistConjF][(snPNew->flipSliceF << 1) + snPNew->parity]][symIdxMultiply[snPNew->symF][32]];
...
int main(int argc, char* argv[])
{
	...
	initEdge4PosMove(); pp();
	initEdge6PosMove(); pp();
	initSymFlipSliceClassMove(); pp(); printf("\n");
